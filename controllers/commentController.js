...
/**
 * function to get all comments On bugs.
 */
let getCommentsByBugId = (req, res) => {

    if (check.isEmpty(req.params.issueId)) {
        logger.error("bugId is missing", "Comment Controller: getCommentsByBugId", 10);
        let apiResponse = response.generate(true, "bugId is missing", 500, null);
        reject(apiResponse);
    }
    else {
        CommentModel.find({ bugId: req.params.bugId }, (err, commentDetails) => {

            /* handle the error if the user is not found */
            if (err) {
                logger.error('Failed to find comments', "Comment Controller: getCommentsByBugId", 10);
                ...
                logger.error("No Comment Found", "Comment Controller: getCommentsByBugId", 10);
                let apiResponse = response.generate(true, "No comment details Found", 500, null);
                res.send(apiResponse);
            }
            else {
                logger.info("Comments found", "Comment Controller: getCommentsByBugId", 10);
...
module.exports = {
    ...
    getCommentsByIssueId:getCommentsByBugId
